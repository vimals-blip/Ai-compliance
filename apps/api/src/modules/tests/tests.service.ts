...
const DEFAULT_TESTS: AutomatedTestItem[] = [
  {
    id: 'test-1',
    code: 'SEC-AWS-001',
...
export class TestsService {
  ...
  update(id: string, body: any) {
    ...
    this.tests = this.tests.map((t) => {
      if (t.id === id || t.id === body.id) {
        updatedItem = { ...t, ...body, lastRun: 'Just now' };
        return updatedItem;
      }
      return t;
    });
    return { success: true, test: updatedItem || body };
  }
}
